...
@Component({
  ...
  templateUrl: './typ-documents.component.html',
  styleUrl: './typ-documents.component.css'
})
export class TypDocumentsComponent {
  ...
  getTypeDocument() {
    this.loading = false;
    this._TypeDocumentsService.getTypeDocument(this.pageSize, this.currentPage).subscribe({
  ...
  async deleteDocument(id: string) {
    return new Promise((resolve, reject) => {
      this._TypeDocumentsService.deleteTypeDocument(id).subscribe({
      next: async (response: any) => {
        if(response) {
