...
@Component({
  ...
  styleUrl: './type-payments.component.css'
})
export class TypePaymentsComponent {
  ...
  openModal(action: string, data: any) {
    ...
    const dialogRef = this._MatDialog.open(TypePaymentsModalComponent, {
      disableClose: true,
  ...
  actionModal(action: string, data: any, msg: string, color: string = '!text-red-500', icon = 'delete') {
    ...
    ref.componentInstance.accept.subscribe(async () => {
      ...
      try {
        await this.deletePayments(data.id)
  ...
  getTypePayments() {
    this.loading = false;
    this._TypePaymentsService.getTypePayments(this.pageSize, this.currentPage).subscribe({
  ...
  async deletePayments(id: string) {
    return new Promise((resolve, reject) => {
      this._TypePaymentsService.deleteTypePayments(id).subscribe({
      next: async (response: any) => {
        if(response) {
